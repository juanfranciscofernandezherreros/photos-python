# flow_layout.py — Layout that places widgets in a row and, when there is
# not enough width, wraps them to the next line (like paragraph text).
# Used instead of QHBoxLayout for button rows so the window is responsive:
# when narrowing the window or using it on small screens, buttons wrap
# instead of being clipped or forcing a huge minimum width.
#
# Adapted from the official Qt "Flow Layout" example (LGPL), ported to PyQt6.
